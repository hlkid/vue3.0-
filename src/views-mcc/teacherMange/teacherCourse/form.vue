    ...
    >
      <div>
        ...
        <el-form-item label="课件数量" prop="courseNum" style="margin-left:220px">
          <el-input v-model="addForm.courseNum" clearable placeholder="请输入课件数量" style="width:350px"></el-input>
        </el-form-item>
      </div>
      <div>
        <el-form-item label="课程类型" prop="type">
          <el-select v-model="addForm.type" placeholder="请选择课程类型" style="width:350px">
            <el-option
  ...
  methods: {
    ...
    confirm() {
      this.$refs["addForm"].validate(validator => {
        if (validator) {
          if (this.dialogFlag == "add") this.addTeacherCourse();
          if (this.dialogFlag == "updata") this.editTeacherCourse();
        }
      });
    },
    // 详情
    ...
    // 新增
    addTeacherCourse() {
      addTeacherCourse(this.addForm).then(res => {
        if (res.code == 200) {
    ...
    editTeacherCourse() {
      ...
      editTeacherCourse(this.addForm).then(res => {
        if (res.code == 200) {
