  ...
  <section class="view-container">
    ...
    <section class="list-wraper">
      <el-row class="btn-wraper">
        <el-button size="small" type="primary" @click="handleAdd" v-if="hasButton('volunteer_config_dictionary_add')"><i class="fa-fw fa fa-plus"></i>添加</el-button>
      </el-row>
            ...
            <el-table-column label="操作" width="250">
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="primary"
                  @click="handleAdd(scope.row)" v-if="hasButton('volunteer_config_dictionary_child_add')"><i class="fa fa-plus fa-fw"></i>添加子级</el-button>
                <el-button
                  size="mini"
                  @click="handleEdit(scope.row)" v-if="hasButton('volunteer_config_dictionary_edit')"><i class="fa fa-pencil fa-fw"></i>编辑</el-button>
                <el-button
                  size="mini"
                  type="danger"
                  @click="handleDelete(scope.row)" v-if="hasButton('volunteer_config_dictionary_delete')"><i class="fa fa-trash-o fa-fw"></i>删除</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <el-dialog :title="dialogType == 'add' ? '添加' : '编辑'" :visible.sync="dialogShow" width="600px" :close-on-click-modal="false" :append-to-body='true'>
      <el-form class="dialog-form" ref="dialogForm" :model="dialogForm" label-width="100px" :rules="rules">
        ...
        <el-form-item label="字典值名称" prop="valueName">
          <el-input v-model="dialogForm.valueName" clearable placeholder="字典值名称" maxlength="20"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogShow = false">取 消</el-button>
        <el-button type="primary" @click="sure" :disabled="vloading">确 定</el-button>
      </div>
    </el-dialog>
  </section>
</template>
...
<script>
  import { addDictionary,deleteDictionary,editDictionary,getDictionaryChildes,getDictionaryDataPage,getDicTypeCodeEnum } from '@/api/volunteer/volunteer'
