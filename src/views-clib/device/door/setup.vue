<template>
  <section class="view-container">
    ...
    <el-form class="hbsx-form" ref="dataForm"
      ...
      <el-form-item label="开门方式" prop="type" class="block">
        <el-checkbox-group v-model="dataForm.openMode">
          <el-checkbox v-for="item in openModeOption" :label="item.value" :key="item.key">{{item.label}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      this.getInfoDevice();
      this.applicationDictionaryDataByTypeCode(this.$dicts.openDoorWay,this.openModeOption);
    },
    methods: {
        ...
        getInfoDevice(this.id).then(res => {
          if(res.code == 200) {
            ...
            if (res.data.openMode) {
              this.dataForm.openMode = res.data.openMode.split(',').map(e => {
                return parseInt(e)
              });
            }else{
