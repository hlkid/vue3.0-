  ...
  <section class="view-container">
    ...
    <section class="list-wraper">
      <el-row class="btn-wraper">
        <el-button size="small" type="primary" v-if="hasButton('device_seat_add')" @click="handleAdd"><i class="fa-fw fa fa-plus"></i>新增
        </el-button>

        <el-button size="small" type="primary" v-if="hasButton('device_seat_excel')" @click="handleExcel"><i class="fa-fw fa fa-pencil-square-o"></i>导出
        </el-button>

        <el-button size="small" type="primary" v-if="hasButton('device_seat_gave')" @click="handleGave(currentSelect)"><i class="fa-fw fa fa-plus"></i>查看智能灯
        </el-button>

        <el-button size="small" type="primary" v-if="hasButton('device_seat_setup')" @click="handleSetup(currentSelect)"><i class="fa-fw fa fa-plus"></i>设置
        </el-button>
      </el-row>
        ...
        <template>
          <el-table
            ...
            <el-table-column label="操作" width="450">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" v-if="hasButton('device_seat_sync')" @click="hanldSync(scope.row)"><i
                  class="fa fa-pencil fa-fw"></i>设备同步
                </el-button>
                <el-button size="mini" type="primary" v-if="hasButton('device_seat_look')" @click="hanldLook(scope.row)"><i
                  class="fa fa-pencil fa-fw"></i>查看
                </el-button>

                <el-button size="mini" v-if="hasButton('device_seat_fault')" :type="scope.row.faultStatus==0?'info':'primary'" @click="hanldSetFault(scope.row)"><i
                  class="fa fa-pencil fa-fw"></i>{{scope.row.faultStatus==0?'故障':'正常'}}
                </el-button>

                <el-button size="mini" v-if="hasButton('device_seat_status')" :type="scope.row.status == 1?'success':'info'" @click="handleEndble(scope.row)"><i
                  class="fa fa-ban fa-fw"></i>{{scope.row.status == 1?'启用':'禁用'}}
                </el-button>

                <el-button size="mini" type="primary" v-if="scope.row.status == 1 && hasButton('device_seat_edit')"  @click="hanldEdid(scope.row)"><i
                  class="fa fa-pencil fa-fw"></i>编辑
                </el-button>

                <el-button size="mini" type="danger" v-if="scope.row.status == 1  && hasButton('device_seat_delete')" @click="hanldDelete(scope.row)"><i
