...
<!--              <el-input  v-model="item.curatorName" placeholder="" :disabled="isDisable"></el-input>-->
<!--              :filter-method="dataFilter"-->
              <el-select clearable v-model="item.curatorId"  @change="selectChanged(item)"   placeholder="请选择推荐人" :disabled="isDisable">
                <el-option
            ...
            <el-form-item label="推荐人简介" >
              <el-input
                type="textarea"
                :autosize="{ minRows: 4, maxRows: 6}"
                placeholder="请输入推荐人简介"
                v-model="item.recommendedSummary"  disabled>
              </el-input>
            </el-form-item>
            <el-form-item label="推荐语">
              <el-input
                ...
                :autosize="{ minRows: 4, maxRows: 6}"
                placeholder="请输入推荐语"
                v-model="item.recommendedLanguage"  :disabled="isDisable">
              </el-input>
            </el-form-item>
...
<script>
  ...
  import {  getRecords,bookType,booksRecord,create,libChange,deleteRecord,getTeacherDemeanorData } from "@/api/clib/book";
    ...
    methods:{
      ...
      selectChanged(item) {
        ...
        this.teachList.map(e => {
          if(e.value == item.curatorId){
            that.$set(item,'recommendedSummary',e.introduce)
          } else {
            return ;
          }
        });
      ...
      updateBookInfo(item){
        ...
        if(item.curatorId==null||item.curatorId==""){
          alert('推荐人不能为空！');
          return;
        }
        if(item.recommendedLanguage==null||item.recommendedLanguage==""){
          alert('推荐理由不能为空！');
          return;
        }
        ...
        var param =  {
          ...
          recommendedSummary:item.recommendedSummary,
          recommendedLanguage:item.recommendedLanguage,
      ...
      //获取推荐人
      getTeacher(){
        getTeacherDemeanorData({name:''}).then(res=>{
          if (res.code == 200){
            this.teachList = res.data.map(e => {
              e.label = e.name;
              e.value = e.id;
              e.introduce = e.introduce
              return e;
            });
          }
        })
