...
<!--        <el-input clearable v-model.trim="searchForm.instId" placeholder="输入场馆名称"></el-input>-->
...
<!--              >已驳回</el-link>-->
    ...
    <!--新增对话框-->
    <el-dialog :title="dialogType == 'add' ? '新增' : '编辑'" :visible.sync="dialogVisible" width="30%"  :append-to-body='true'>
...
<script>
  ...
  import { getRecords,getPeriod,getAuthStatus,save,sendExamine,examine,publish,deleteById,detail,update} from "@/api/clib/book";
  ...
  export default {
    ...
    methods:{
      ...
      //好书推荐列表
      getList(){
        getPeriod(this.searchForm).then(res=>{
          if (res.code == 200){
      ...
      //查看 type 为 1代表是查看
      handleView(row){
        this.$router.push({name:'book_recommend_edit',params:{type:1,id:row.id}})
      },
      //好书推荐
      bookRecommend(row){
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            var param = {};
            param = this.form;
            param.periodType = 2  //期次类型 默认2 好书推荐)
           console.log(param)
            save(param).then(res=>{
              if (res.code == 200){
