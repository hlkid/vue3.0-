...
<script>
  ...
  import {getRecords, booksNewList,getAuthStatus,save,sendExamine,examine,publish,deleteById,detail,update} from "@/api/clib/book";
  ...
  export default {
    ...
    methods:{
      ...
      //好书推荐列表
      getList(){
        booksNewList(this.searchForm).then(res=>{
          if (res.code == 200){
      ...
      //查看 type 为 1代表是查看
      handleView(row){
        this.$router.push({name:'book_new_look',params:{type:1,id:row.id}})
      },
      //图书管理
      bookRecommend(row){
        this.$router.push({name:'book_new_edit',params:{id:row.id}})
      },
      ...
      handleAdd(){
        ...
        for (let key in this.form){
          console.log(this.venueList[0].institutionId)
