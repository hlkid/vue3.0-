...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //查看 type 为 1代表是查看
      handleView(row){
        this.$router.push({name:'book_manage_look',params:{type:1,id:row.id}})
      },
      //好书推荐
      bookRecommend(row){
        this.$router.push({name:'book_manage_edit',params:{id:row.id}})
      },
      ...
      //新增
      handleAdd(){
        this.$router.push({name:'book_select_add'})
      },
